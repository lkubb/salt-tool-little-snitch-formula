...
We will see a quick setup to configure the ``tool_snitch`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_snitch`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_snitch`` formulas under ``/srv/salt/tool_snitch/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_snitch/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_snitch/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_snitch/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_snitch/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_snitch`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_snitch`` formulas under ``/srv/salt/tool_snitch/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_snitch/parameters/roles
...
We create a configuration for the role ``tool_snitch/server`` in ``/srv/salt/tool_snitch/parameters/roles/tool_snitch/server.yaml``:
...
We create another configuration for the role ``tool_snitch/client`` in ``/srv/salt/tool_snitch/parameters/roles/tool_snitch/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_snitch/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_snitch/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_snitch/parameters/dns:domain/  '/srv/salt/tool_snitch/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-snitch-as-json:
      test.nop:
        - name: {{ snitch | json }}
...
.. code-block:: sls
    ...
    snitch-config-file-file-managed:
      file.managed:
        - name: {{ snitch.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='snitch-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ snitch.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``snitch`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ snitch['winner'] }}
